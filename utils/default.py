...
def create_model(args):
    print('model num_classes: ', args.num_classes)
    if 'wideresnet' in args.arch:
        ...
        print('resnet_imagenet num_classes: ', args.num_classes)
...
def set_dataset(args):
    labeled_dataset, unlabeled_dataset, test_dataset, val_dataset, unique_labeled_dataset = \
        DATASET_GETTERS[args.dataset](args)

    ood_loaders = {}
    if args.no_extra_ood:
        args.ood_data = []
    for ood in args.ood_data:
        ood_dataset = get_ood(args, ood, args.dataset, image_size=args.image_size)
    ...
    unique_labeled_trainloader = DataLoader(
        unique_labeled_dataset,
        sampler=train_sampler(unique_labeled_dataset),
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        drop_last=True)
    ...
    return labeled_trainloader, unlabeled_dataset, \
           test_loader, val_loader, ood_loaders, unique_labeled_trainloader
