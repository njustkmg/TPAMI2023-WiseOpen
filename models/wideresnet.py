...
class WideResNet_Open(nn.Module):
    def __init__(self, num_classes, depth=28, widen_factor=2, drop_rate=0.0):
        ...
        self.mlp_proj = nn.Sequential(*[
            ...
            nn.Linear(channels[3], proj_size)
        ])
        self.mb_classifiers = nn.Linear(proj_size, 2 * num_classes, bias=False)
    ...
    def forward(self, x, feature=False, feat_only=False):
        ...
        if feat_only:
            return out
        feat_proj = self.mlp_proj(out)
        out_mb = self.mb_classifiers(feat_proj)
        out_open = self.openset_classifier(feat_proj)

        if feature:
            return self.fc(out), out_mb, out_open, out
        else:
            return self.fc(out), out_mb, out_open
